...
name: guidance
version: "1.2.0"
description: "回答用户关于 QingwenClaws 安装与配置的问题。优先定位并阅读本地文档，再提炼答案；文档不足时访问官网。"
dependencies:
  tools:
    - readMateClawDoc
    - read_file
...
# QingwenClaws 安装与配置问答

当用户询问 QingwenClaws 的安装、初始化、环境配置、依赖要求、常见配置项时使用本技能。
...
如本地文档信息不足：
```
search(query="QingwenClaws 安装配置 <关键词>", language="zh-CN", count=5)
```
