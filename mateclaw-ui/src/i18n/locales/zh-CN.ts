export default {
  ...
  settings: {
    ...
    multimodalDesc: '统一管理图片生成、语音合成 / 识别、视频生成、音乐生成和 3D 模型生成的 Provider 配置',
    searchTitle: '搜索服务',
  ...
  agents: {
    ...
    intro: {
      title: '什么是数字员工？',
      body: '数字员工是一个被你雇佣并配置过的 AI 角色：选好"大脑"（模型）、给它"专业能力"（技能/工具）、设定"工作守则"（提示词），它就能在对话、渠道、定时任务里替你干活。',
      bullets: {
        a: '一个员工 = 模型 + 技能 + 工具 + 提示词 + 记忆',
        b: '可以同时在多个渠道（钉钉/飞书/网页）上岗',
        c: '出错可回溯：每一次决策、每一次工具调用都有审计',
      },
      footer: '新员工 → 选模型 → 绑技能 → 测试对话 → 接入渠道，完整闭环只需要 4 步。',
      hide: '不再提示',
      show: '了解什么是数字员工',
    },
    newAgent: '新员工',
    ...
    activity: {
      ...
      intro: {
        ...
        bullets: {
          ...
          c: '点击任意一行可查看完整请求/响应/上下文',
        },
        footer: '排查"为什么员工那次回错了"或"谁动了模型配置"时，先来这里看。',
        hide: '不再提示',
        show: '了解什么是活动记录',
      },
      allActions: '全部操作',
  ...
  plugins: {
    ...
    intro: {
      ...
      bullets: {
        ...
        c: '适合做内部系统对接、私有 API 调用、定制业务逻辑',
      },
      footer: '若你只需简单脚本，建议优先用「技能」；需要深度集成 Java 生态时再选插件。',
      hide: '不再提示',
      show: '了解什么是插件',
    },
    refresh: '刷新',
    ...
    noDescription: '暂无描述',
    emptyTitle: '暂无已安装插件',
    emptyHint: '将插件 JAR 文件放入 ~/.qingwenclaws/plugins/ 目录后重启服务',
    loading: '加载插件中...',
    loadFailed: '加载插件失败',
    enabled: '{name} 已启用',
    disabled: '{name} 已禁用',
  ...
  cronJobs: {
    ...
    intro: {
      title: '什么是定时任务？',
      body: '定时任务用 Cron 表达式让数字员工按时自动跑：可以是每天早上汇报、每周一巡检、整点抓数据，也可以是一次性的预约执行。',
      bullets: {
        a: '触发后等价于"有人发了一条消息"，员工照常完成对话流程',
        b: '多实例部署也只会执行一次（分布式锁保护）',
        c: '可随时启用/停用/补跑，历史结果都能追溯',
      },
      footer: '提示：把员工调好（在对话里能跑通），再把同样的输入复制成定时任务，最稳。',
      hide: '不再提示',
      show: '了解什么是定时任务',
    },
    createJob: '新建任务',
  ...
  workflows: {
    kicker: '工作流',
    title: '工作流编辑器',
    desc: '把多步业务编排成自动化流水线：指定每一步由哪个员工执行，支持并行、条件分支、人工审批和跨渠道推送。',
    intro: {
      title: '工作流能做什么？',
      body: '数字员工是一个人——你跟它聊，它自己想办法。工作流是一套 SOP——你提前定好每一步由谁做、结果传给谁、出错怎么办，触发后自动从头跑到尾。',
      bullets: {
        a: '串联多个员工协作：A 写初稿 → B 审校 → C 翻译 → 推到钉钉群，全程无人值守',
        b: '卡审批：流程走到某步暂停，等指定人在系统里点「通过」才继续',
        c: '自动触发：定时跑、收到消息触发、上游流程跑完自动接力',
      },
      footer: '能提前画出流程图 → 建工作流；画不出来、需要 AI 自己想 → 直接找员工对话。',
      hide: '不再提示',
      show: '工作流能做什么？',
    },
    newWorkflow: '+ 新建工作流',
    ...
    templates: {
      label: '插入模板',
      placeholder: '选择步骤模板',
      sequential: '顺序步骤 — 上一步做完接着做',
      fan_out: '并行扇出 — 多个步骤同时跑',
      collect: '汇聚结果 — 等并行步骤都完成后合并',
      conditional: '条件分支 — 满足条件才执行',
      await_approval: '等待审批 — 暂停，等人批准后继续',
      dispatch_channel: '消息分发 — 推送到钉钉/飞书等渠道',
      write_memory: '写入记忆 — 存入员工的记忆文件',
    },
    ...
    canvas: {
      ...
      modeHints: {
        sequential: '上一步做完接着做',
        fan_out: '多个步骤同时并行跑',
        collect: '等并行步骤都完成，合并结果',
        conditional: '满足条件才执行这一步',
        await_approval: '暂停流程，等指定人批准后继续',
        dispatch_channel: '把结果推送到钉钉/飞书等渠道',
        write_memory: '把结果存入员工的记忆文件',
      },
      inspector: {
        title: '步骤属性',
        empty: '点击画布上的节点查看详情。',
  ...
  channels: {
    ...
    intro: {
      title: '什么是渠道？',
      body: '渠道是数字员工"对外说话"的入口：钉钉群、飞书机器人、企业微信、网页聊天、Webhook 等。配好渠道后，员工就能在那个平台上自然地收发消息、推送结果。',
      bullets: {
        a: '一个员工可以绑定多个渠道，同时在多平台上岗',
        b: '渠道之间互相隔离，单个出问题不影响其他',
        c: '消息流自动审计，发了什么、谁触发、何时回复都可查',
      },
      footer: '新接入建议：先用网页 Chat 调通员工，再绑定 IM 渠道做内测，最后开放给业务。',
      hide: '不再提示',
      show: '了解什么是渠道',
    },
    newChannel: '新建渠道',
  ...
  skills: {
    ...
    aiCreateSkill: 'AI 建技能',
    aiCreateSkillTooltip: '通过自然语言对话，让 AI 一步步帮你创建新技能',
    aiCreateSkillOpening: '正在打开...',
    aiCreateSkillDisabled: '「技能创建助手」当前已被禁用，请到数字员工页启用后再试',
    skillBuilderMissingTitle: '未找到「技能创建助手」',
    skillBuilderMissingDesc: '「技能创建助手」数字员工不存在或已被删除。\n\n如需继续使用 AI 建技能功能，请到「数字员工」页重新创建该员工，并满足以下配置：\n\n• 名称：技能创建助手\n• Agent 类型：react\n• 绑定技能：skill-creator（系统内置）\n• 授权工具：skill_manage\n\n是否前往数字员工页？',
    skillBuilderGoCreate: '去创建数字员工',
    ...
    intro: {
      ...
      bullets: {
        ...
        c: '像装 App 一样，给数字员工灵活扩展能力',
      },
      footer: '在下方按分类浏览，点击「启用」加入可用库，再到数字员工设置里绑定。',
      hide: '不再提示',
      show: '了解什么是技能',
    },
    groups: {
  ...
  memory: {
    ...
    intro: {
      title: '什么是记忆？',
      body: '记忆是数字员工的"长期大脑"：把你和它的对话沉淀成可复用的事实、偏好、流程，下次再聊不必从零开始。系统会自动抽取与定期整理，也支持手动编辑。',
      bullets: {
        a: 'AGENTS.md / PROFILE.md / SOUL.md — 角色与用户画像',
        b: 'MEMORY.md — 日常事实、偏好与项目上下文',
        c: '夜间巩固（Dreaming）会自动去重、合并、清理过期条目',
      },
      footer: '记忆是员工"越用越好用"的关键，可以随时人工干预，确保它没记错或没漏掉关键信息。',
      hide: '不再提示',
      show: '了解什么是记忆',
    },
    selectAgent: '选择智能体',
  ...
  analytics: {
    ...
    addField: '添加字段',
    editField: '编辑字段',
    semanticHint: '这段文字会告诉数据分析专家这一列的业务含义和口径，建议填写',
    fieldCode: '字段编码',
    fieldName: '字段名称',
    fieldType: '字段类型',
    excelHeader: 'Excel表头',
